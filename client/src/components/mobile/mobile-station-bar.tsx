...
export default function MobileStationBar({
  ...
  const previewStation = selectedStation || (stations.length > 0 ? stations[0] : null);
  
  // Fetch ports for selected/preview station
  const { data: ports } = useQuery({
    queryKey: previewStation ? [`/api/stations/${previewStation.id}/ports`] : null,
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white z-20 shadow-lg rounded-t-lg">
      {/* Header */}
      ...
      {!isListVisible && (
        <div className="p-4">
          <div className="flex justify-between">
            <h3 className="text-base font-medium text-gray-900">{previewStation.name}</h3>
          ...
          <div className="mt-2 flex justify-between">
            <div className="flex items-center text-sm text-gray-500">
              ...
              <span>{previewStation.powerKw}kW {previewStation.connectorTypes?.join(", ")}</span>
            </div>
            <span className="text-sm text-gray-600">{previewStation.distance} mi</span>
          </div>
          <Button className="mt-3 w-full" onClick={() => onSelectStation(previewStation)}>
            View Details & Book
          </Button>
        </div>
      )}
        ...
        <div className="max-h-96 overflow-y-auto">
          {stations.map((station) => (
            <div 
              key={station.id}
              className={cn(
                "p-3 border-b border-gray-100 flex justify-between items-center cursor-pointer",
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-600">{station.powerKw}kW · {station.distance} mi</p>
              </div>
              <Badge variant={getBadgeVariant() as any} className="text-xs font-medium">
                {availablePorts.length === 0 
                  ? "Fully Booked" 
